...
MAPPING_YAML = './conf/mappings/mapping.yaml'

# define some regular expressions we use later
IPv4_REGEX = r"ip\saddress\s(\S+\s+\S+|\S+\/\d+)"
DHCP = r"ip\saddress\sdhcp"
LAG = r"\s+channel-group\s(\d+)\smode\s(\S+)$"
OSPF_ROUTER_ID = r"^\s+router-id\s(\S+)"
HOSTNAME = r"^hostname\s+(\S+)"
DOMAIN_NAME = r"^ip\s+domain\s+name\s+(\S+)"
CHANNEL_GROUP = r"^\s+channel-group"
VLAN_NAME = r"^\s+name (\S+)"
ACCESS = r"^\s+switchport mode access"
TRUNK = r"^\s+switchport mode trunk"
SWITCHPORT_ACCESS_VLAN = r"^\sswitchport access vlan (\d+)"
TRUNK_ALLOWED_VLANS = r"^\s+switchport trunk allowed vlan (\S+)"
...
class DeviceConfig:
    ...
    # some cisco systems uses different spellings like Port-channel vs. port-channel
    __spelling = {}
    ...
    def __init__(self, raw):
        ...
        self.__deviceConfig = CiscoConfParse(self.__raw.splitlines())
    ...
    def __parse_config(self):
        ...
        for interface_cmd in interface_cmds:
            ...
            self.__config["interfaces"][intf_name]["description"] = None
            self.__config["interfaces"][intf_name]["type"] = self.get_interface_type(intf_name.lower())

            # save spelling
            if 'Port-channel' in intf_name:
                self.__spelling["port-channel"] = "Port-channel"
            if 'port-channel' in intf_name:
                self.__spelling["port-channel"] = "port-channel"

            # get description
            for cmd in interface_cmd.re_search_children(r"^\s+description\s"):
                self.__config["interfaces"][intf_name]["description"] = cmd.text.strip()[len("description "):]

            # check if interface is a SVI; add SVI to list of VLANs
            if "vlan" in intf_name.lower():
                vid = intf_name.replace("Vlan","")
                for cmd in interface_cmd.re_search_children(r"^\s+description\s"):
                    name = cmd.text.strip()[len("description "):]
                self.__config["vlan"][vid]['vid'] = vid
                self.__config["vlan"][vid]['name'] = name
                self.__list_of_vlans.add(vid)
    ...
    def get_section_by_regular_expression(self, pattern):

        response = []
        match = re.finditer(pattern, self.__raw, flags=re.DOTALL | re.MULTILINE)
        if match:
            for m in match:
                ...
                if len(m.groupdict()) == 0:
                    ...
                    for gd in m.groupdict():
                        # we create a dict with the matched pattern as name
                        # eg. "GigabitEthernet0/0": [ lines of config here] }
                        response.append({m.groupdict(gd)[gd]: m.group().splitlines()})
        return response

    def get_section_by_object(self, pattern):

        response = []
        # maybe I did understood something wrong ;-( All I want is all children of a parent eg an interface
        # but nxos uses multiple children like hsrp that has children itself
        # so we get a list of all patterns and then generate blocks that contain the config lines
        list_of_all_parents = self.__deviceConfig.find_objects(r"%s" % pattern)
        for child in list_of_all_parents:
            lines = self.__deviceConfig.find_all_children(r"%s$" % child.text)
            block = []
            for line in lines:
                block.append(line)
            response.append(block)
    ...
    def get_section_as_dict_by_object(self, pattern, name_pattern, group_name):
        ...
        for child in list_of_all_parents:
            ...
            for line in lines:
                ...
                block.append(line)
            response.append({name: block})

        return response

    def get_interface_spelling(self, word):
        return self.__spelling.get(word)
